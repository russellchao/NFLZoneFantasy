export const fetchScheduleByTeam = async (teamName) => {
    try {
        const response = await fetch(
           `${process.env.REACT_APP_SPRING_URL}/api/v1/schedule?teamName=${encodeURIComponent(teamName)}`
        );
...
export const fetchScheduleByWeek = async (weekNum) => {
    try {
        const response = await fetch(
            `${process.env.REACT_APP_SPRING_URL}/api/v1/schedule?weekNum=${encodeURIComponent(weekNum)}`
        );
...
export const fetchScheduleByMatchup = async (team1, team2) => {
    try {
        const response = await fetch(
            `${process.env.REACT_APP_SPRING_URL}/api/v1/schedule/specific?team1=${encodeURIComponent(team1)}&team2=${encodeURIComponent(team2)}`
        );
